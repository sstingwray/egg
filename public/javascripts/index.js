(function() {
    ...
    let app = {
        ...
        gizmoPatterns: {
            ...
            /*darkestAgesBenefactors: {
                title: 'Темнейшие Века: Компаньон',
                class: 'darkest-ages-benefactors',
                btnPanel: '.darkest-ages-benefactors-btn-panel',
                knobPanel: '.darkest-ages-benefactors-knob-panel',
            }*/
        },
        components: {
            spinner: {},
        ...
        marketEconSimModule: {
            ...
            productionRuleSets: {
                trader: {
                    name: 'Торговец',
                    tooltip: 'Зарабатывает своим пониманием ситуации на рынке',
                    rules: [],
                },
                farmer: {
                    name: 'Фермер',
                    tooltip: 'Зарабатывает направляя рабочую силу в производство пищи',
                    rules: [
                        {
                            name: 'farming',
                            components: [
                ...
                supplier: {
                    name: 'Снабженец',
                    tooltip: 'Зарабатывает снабжая рабочую силу необходимым снаряжением',
                    rules: [
                        {
                            name: 'professional',
                            components: [
                ...
                recruiter: {
                    name: 'Рекрутер',
                    tooltip: 'Зарабатывает подготовкой рабочей силе',
                    rules: [
                        {
                            name: 'mass',
                            components: [
                ...
                refiner: {
                    name: 'Шахтер',
                    tooltip: 'Зарабатывает направляя рабочую силу в производство сырья',
                    rules: [
                        {
                            name: 'refining',
                            components: [
                ...
                smelter: {
                    name: 'Промышленник',
                    tooltip: 'Зарабатывает трансформирую сырье в сплавы',
                    rules: [
                        {
                            name: 'smelting',
                            components: [
                ...
                artisan: {
                    name: 'Ремесленник',
                    tooltip: 'Зарабатывает трансформируя сплавы в товары',
                    rules: [
                        {
                            name: 'manufacturing',
                            components: [
    ...
    app.fabricateNewEconAgent = (type = 'agent') => {
        ...
        switch (type) {
            case 'bank':
                //bank only trades and jump-starts the economy by selling basic goods at a current mean price
                newAgent.name = 'Банк';
                newAgent.type = 'trader';
                newAgent.protected = true;
                newAgent.wallet = 0;
                for (let goodKey of Object.keys(newAgent.inventory)) {
                    newAgent.inventory[goodKey] = 0 + app.marketEconSimModule.goods[goodKey].baseCost*100;
                };
            ...
            default:
                ...
                for (let goodKey of Object.keys(newAgent.inventory)) {
                    newAgent.inventory[goodKey] = round(Math.random()*10 + 1, 0);
                    newAgent.priceBeliefs[goodKey].upperLimit = app.marketEconSimModule.market.goodHistoricalMeans[goodKey]*round((2 + Math.random()), 2);
                    newAgent.priceBeliefs[goodKey].lowerLimit = app.marketEconSimModule.market.goodHistoricalMeans[goodKey]/round((2 + Math.random()), 2);
                };
                break;
        };
    ...
    app.marketTurnSimulation = async () => {
        let gatherListings = async () => {
            ...
            let createGoodListing = async (agent, goodKey) => {
                //Desired amount is 10 turns of upkeep + 10 of all agent components to enable production. If an agent is the bank, they always want to sell and never buy. If its a player, the amounts are taken from the player request panel
                let playerRequestAmount = parseInt(playerBillboard.querySelector(`.${goodKey}-field > .amount`).value);
                let upkeepAmount = agent.upkeepRules.filter(x => x.good == goodKey).reduce((a, b) => a + b.quantity, 0);
                let productionAmount = (app.marketEconSimModule.productionRuleSets[agent.type].rules.filter(x => x.components.filter(y => y.key== goodKey).length > 0).length > 0 ? 100 : 0);
                ...
                } else  transactionAmount = agent.inventory[goodKey] - upkeepAmount*10 - productionAmount - 10;
                ...
                switch (transactionType) {
                    case 'lot':
                        {
                            ...
                            let upperPriceLimit = (!agent.protected ? agent.priceBeliefs[goodKey].upperLimit : parseFloat(playerBillboard.querySelector(`.${goodKey}-field > .max-price`).value));
                            let lowerPriceLimit = (!agent.protected ? agent.priceBeliefs[goodKey].lowerLimit : parseFloat(playerBillboard.querySelector(`.${goodKey}-field > .min-price`).value));
                            //if mean price is higher then my upper price for a good, its time to sell everything, if mean is lower then my lower price, i'm not gonna sell
                            let favorability = (agent.protected ? 1 : (mean > upperPriceLimit ? 1 : (mean < lowerPriceLimit ? 0 : Math.max(0, findNumberPosition(mean, lowerPriceLimit, upperPriceLimit)))));
                            let amount = round(favorability*Math.abs(transactionAmount), 0);
                    ...
                    case 'ask':
                        { 
                            ...
                            //if mean price is higher then my upper price for a good, i'm gonna buy the very minimum, if mean is lower then my lower price, i'm gonna buy all i need...
                            ...
                            let amount = round(Math.min(favorability*Math.abs(transactionAmount), agent.wallet/upperPriceLimit), 0);

                            if (amount > 0) {
                                //console.log(`${agent.name} wants to ${transactionType} ${transactionAmount} of ${goodKey} (${productionAmount} for production, ${upkeepAmount*10} for upkeep)`);
        ...
        let matchListings = async () => {
            ...
            let matchGood = async (goodKey) => {
                ...
                let supply = round(sortedLots.reduce((a, b) => a + b.price*b.quantity, 0), 2);
                let demand = round(sortedAsks.reduce((a, b) => a + b.price*b.quantity, 0), 2);
                ...
                let iterateDeals = async (i, j) => {
                    if (i >= sortedLots.length || j >= sortedAsks.length) return;
                        ...
                        DealMatching: if (quantity >= 1) {
                            ...
                            if (sellerAgent.name == 'Игрок') app.marketEconSimModule.market.currentPlayerMessages.push({ turn: app.marketEconSimModule.market.currentTurn, text: `Игрок смог продать ${quantity} [${app.marketEconSimModule.goods[goodKey].name}] ${clearingPrice*quantity}(${clearingPrice} за шт.), покупатель - ${buyerAgent.name}`});
                            if (buyerAgent.name == 'Игрок') app.marketEconSimModule.market.currentPlayerMessages.push({ turn: app.marketEconSimModule.market.currentTurn, text: `Игрок смог купить ${quantity} [${app.marketEconSimModule.goods[goodKey].name}] ${clearingPrice*quantity}(${clearingPrice} за шт.), продавец - ${sellerAgent.name}`});
                        };

                        PriceBeliefsAdjustment: {
                            if (!buyerAgent.protected) {

                                if (buyer.quantity*0.5 > quantity) {
                                    //if i got less then half of what i wanted, increase both my price beliefs by 10%
                                    buyerAgentPriceBeliefs.lowerLimit += round(buyerAgentPriceBeliefs.lowerLimit*0.1, 2);
                                    buyerAgentPriceBeliefs.upperLimit += round(buyerAgentPriceBeliefs.upperLimit*0.1, 2);
                                } else if (buyerAgentPriceBeliefs.upperLimit > clearingPrice) {
                                    //if i got at least a half of what i wanted and my upper price limit was higher then the clearing price, decrease my upper price belief by 20% of the difference but no lower then my lower price
                                    buyerAgentPriceBeliefs.upperLimit = Math.max(buyerAgentPriceBeliefs.upperLimit - round((buyerAgentPriceBeliefs.upperLimit - clearingPrice)*0.2, 2), buyerAgentPriceBeliefs.lowerLimit);
                                } else if (buyerAgentPriceBeliefs.upperLimit > app.marketEconSimModule.market.goodHistoricalMeans[goodKey]) {
                                    //if i got at least a half of what i wanted and my upper price limit was lower then the clearing price, but higher then historical mean price, decrease my upper price belief by 10% of the difference but no lower then my lower price  
                                    buyerAgentPriceBeliefs.upperLimit = Math.max(buyerAgentPriceBeliefs.upperLimit - round((buyerAgentPriceBeliefs.upperLimit - app.marketEconSimModule.market.goodHistoricalMeans[goodKey])*0.2, 2), buyerAgentPriceBeliefs.lowerLimit);
                                };
                            ...
                            if (!sellerAgent.protected) {
                                ...
                                if (quantity == 0 && seller.quantity > 0) {
                                    //i couldn't sell all my goods, decrease my price beliefs by 10% but no lower then 1 cent
                                    sellerAgentPriceBeliefs.lowerLimit = Math.max(sellerAgentPriceBeliefs.lowerLimit - round(sellerAgentPriceBeliefs.lowerLimit*0.1, 2), 0.01);
                                    sellerAgentPriceBeliefs.upperLimit = Math.max(sellerAgentPriceBeliefs.upperLimit - round(sellerAgentPriceBeliefs.upperLimit*0.1, 2), sellerAgentPriceBeliefs.lowerLimit);
                                } else if (sellerAgentPriceBeliefs.upperLimit < clearingPrice) {
                                    //i sold something but the clearing price was higher then my upper price, increase my price beliefs by 20% of the difference
                                    sellerAgentPriceBeliefs.lowerLimit += round((clearingPrice - sellerAgentPriceBeliefs.lowerLimit)*0.2, 2);
                                    sellerAgentPriceBeliefs.upperLimit += round((clearingPrice - sellerAgentPriceBeliefs.upperLimit)*0.2, 2);
                                } else if (sellerAgentPriceBeliefs.lowerLimit > clearingPrice) {
                                    //i sold something, clearing price was within my upper price but lower then my lower price, decrease my lower price by 10% of the difference
                                    sellerAgentPriceBeliefs.lowerLimit = Math.max(sellerAgentPriceBeliefs.lowerLimit - round((sellerAgentPriceBeliefs.lowerLimit - clearingPrice)*0.1, 2), 0.01);
                                };
                            };
                        ...
                        if (seller.quantity > 0) await iterateDeals(i, j + 1)
                        else if (buyer.quantity > 0) await iterateDeals(i + 1, j)
                        else return;
                    };
                };
                ...
                app.marketEconSimModule.market.goodHistoricalDemand[goodKey] = demand;
                app.marketEconSimModule.market.goodHistoricalSupply[goodKey] = supply;
                app.marketEconSimModule.market.goodHistoricalVolume[goodKey] = round(app.marketEconSimModule.market.currentDeals.filter(x => x.turn == app.marketEconSimModule.market.currentTurn && x.good == goodKey).reduce((a, b) => a + b.clearingPrice*b.quantity, 0), 2);
                topGoods.push({
                    good: goodKey,
                    demand: demand,
                    supply: supply,
            ...
            for (let agent of app.marketEconSimModule.market.currentAgents.filter(x => x.name !== 'Банк' && x.name !== 'Игрок')) {
                for (let goodKey of Object.keys(agent.lastTurnMarketShares)) {
                    let agentRecentDeals = app.marketEconSimModule.market.currentDeals.filter(x => x.turn == app.marketEconSimModule.market.currentTurn && x.seller == agent.name && x.good == goodKey);
                    if (agentRecentDeals.length > 0) {
                        
                        agent.lastTurnMarketShares[goodKey] = agentRecentDeals.reduce((a, b) => a + b.clearingPrice*b.quantity, 0);
                    };
                };
            };

            app.marketEconSimModule.market.topGoodType = topGoods.sort((a, b) => (a.demand < b.demand) ? 1 : -1)[0].good;

            //console.log('Demand for this turn');
            //console.log(app.marketEconSimModule.market.goodHistoricalDemand);
            //console.log('Supply for this turn');
            //console.log(app.marketEconSimModule.market.goodHistoricalSupply);
            //console.log('Trade volume for this turn');
            //console.log(app.marketEconSimModule.market.goodHistoricalVolume);
    ...
    app.upkeepSimulation = async () => {
        let checkAgentUpkeepGood = async (agent, upkeep) => {
            if (agent.inventory[upkeep.good] - upkeep.quantity < 0) {
                ...
                agent.durability -= 1;
                //console.log(`${agent.name} could not fulfill their ${upkeep.quantity} of ${upkeep.good} upkeep, losing durability. Current durability is ${agent.durability}`);
            } else {
                agent.inventory[upkeep.good] -= upkeep.quantity;
                //console.log(`${agent.name} fulfilled their ${upkeep.quantity} of ${upkeep.good} upkeep, setting durability to 1. Current durability is ${agent.durability}`);
                (agent.durability < 10 ? agent.durability += 1 : agent.durability = 10);
                
            };
        };

        let bankUpkeep = async(bankAgent, goodKey, deprecationRate) => {
            bankAgent.inventory[goodKey] -= round(bankAgent.inventory[goodKey]*deprecationRate, 0);
        };
        ...
        for await (let agent of app.marketEconSimModule.market.currentAgents) {
            ...
            else if (agent.name == 'Банк') {
                agent.wallet = agent.wallet*0.50;
                for await (let goodKey of Object.keys(agent.inventory)) {
                    await bankUpkeep(agent, goodKey, 0.25);
                };
            };
            agent.age++;
        };
    ...
    app.bailAndRetiretAgents = async () => {
        ...
        let bailOutAgent = async (agent, i) => {
            //Bail-out mechanism, that buys all the materials from the agent at an 80% historical mean price in order to let them participate in the market again
            if (agent.wallet <= 0 && agent.bailOutChance) {
                //console.log(`Agent ${agent.name} is being bailed-out`);
                for await (let goodKey of Object.keys(agent.inventory)) {
                    await buyOutGood(agent, goodKey, 0.8);
                };
            } else if (agent.durability < 0 || (agent.wallet <= 0 && !agent.bailOutChance)) {
                //Retire mechanism, that buys all the materials from the agent at an 50% historical mean price and retires the agent
                //console.log(`Agent ${agent.name} is being retired with durability of ${agent.durability} and wallet of ${agent.wallet}`);
                for await (let goodKey of Object.keys(agent.inventory)) {
    ...
    app.refreshMarketModule = () => {
        ...
        agentCards: {
            for (let agent of app.marketEconSimModule.market.currentAgents) {
                if (!agent.protected) {
                    ...
                    newAgentCard.querySelector('.agent-type > .value').title = app.marketEconSimModule.productionRuleSets[agent.type].tooltip;
        ...
        meanPricesBillboard: {
            ...
            for (let goodKey of Object.keys(app.marketEconSimModule.market.goodHistoricalMeans)) {
                ...
                let averageLowerLimit = round(app.marketEconSimModule.market.currentAgents.map(x => x.priceBeliefs[goodKey].lowerLimit).reduce((a, b) => a + b, 0)/app.marketEconSimModule.market.currentAgents.length, 2);
                let averageUpperLimit = round(app.marketEconSimModule.market.currentAgents.map(x => x.priceBeliefs[goodKey].upperLimit).reduce((a, b) => a + b, 0)/app.marketEconSimModule.market.currentAgents.length, 2);
                ...
                newBillboardItem.querySelector('.value').title = `${averageLowerLimit} — ${averageUpperLimit}`;
        ...
        /*bailOutBillboard: {
            ...
            bailoutBillboard.classList.add('bank-billboard');
            ...
            if ($('.bank-billboard').length == 0) billboardContainer.appendChild(bailoutBillboard);   
        };*/
        ...
        playerAskLotBillboard: {
            ...
            for (let goodKey of Object.keys(app.marketEconSimModule.market.goodHistoricalMeans)) {
                ...
                $(newBillboardItem.querySelector('.min-price')).on('change', () => {
                    playerAgent.priceBeliefs[goodKey].lowerLimit = parseInt(newBillboardItem.querySelector('.min-price').value);
                });

                $(newBillboardItem.querySelector('.max-price')).on('change', () => {
                    playerAgent.priceBeliefs[goodKey].upperLimit = parseInt(newBillboardItem.querySelector('.max-price').value);
                });
        ...
        if (app.marketEconSimModule.market.currentPlayerMessages.length > 0) $('.econ-sim-player-log').html(app.marketEconSimModule.market.currentPlayerMessages.map(x => x.text).join('<br>'));
    ...
    app.econNextTurn = async () => {
        ...
        thisTurnData = JSON.parse(JSON.stringify(app.marketEconSimModule.market));
        console.log(`Turn ${app.marketEconSimModule.market.currentTurn} data:`);
        console.log(thisTurnData);
        app.refreshMarketModule();
    };
    ...
    $(function() {
        ...
        /*{
            let container = document.querySelector(app.gizmoPatterns.darkestAgesBenefactors.btnPanel);
            app.fabricateButton('darkest-ages-benefactors-', 'Сохранить данные', null, null, null, container);
        };
        {
            let container = document.querySelector(app.gizmoPatterns.darkestAgesBenefactors.knobPanel);
        };*/
